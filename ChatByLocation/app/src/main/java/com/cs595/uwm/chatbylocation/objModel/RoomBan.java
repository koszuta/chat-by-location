...
public class RoomBan {
    ...
    private ValueEventListener roomBanListener;
    ...
    private ValueEventListener createBanListenerForSpecificRoom() {
        DatabaseReference dRef = FirebaseDatabase.getInstance().getReference().child("roomUsers").child(roomID);
        return dRef.child("bannedUsers").orderByKey().addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.getKey().equals(userID)) {
                    String isBanned = dataSnapshot.getValue(String.class);
